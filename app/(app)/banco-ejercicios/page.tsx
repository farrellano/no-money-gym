  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-sm rounded-xl bg-zinc-800 p-6 shadow-xl">
        <h2 className="text-lg font-bold text-white">{t.onboardingWelcome}</h2>
        <p className="mt-2 text-sm text-zinc-400">{t.onboardingQuestion}</p>
        <input
          type="text"
          value={name}
    ...
    <div className="p-4 space-y-4">
      ...
      {nombreUsuario && (
        <p className="text-sm text-zinc-400">
          {t.greeting(nombreUsuario)}
        </p>
      )}

      {/* Muscle group filter */}
      <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-none">
        {t.muscleGroupKeys.map((key, i) => (
          <button
            key={key}
      ...
      {ejercicios && ejercicios.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-zinc-500">
          ...
          <p className="mt-2 text-sm">{t.noExercises}</p>
          <p className="text-xs">{t.noExercisesHint}</p>
        </div>
      )}
    </div>
  );
